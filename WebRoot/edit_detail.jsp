...
<head>
    <title>个人信息界面</title>
    <link href="edit_detail.css" type="text/css" rel="stylesheet" />
		...
		<div class="col header__inner">
	        <div class="col header__navwrap">
	          	<div class="col header__navbar">
	          		<a href="" class="col header__navitem">首页</a>
    ...
    String driverClass = "com.mysql.jdbc.Driver";
   	...
   	if(conn != null){
   		...
   		if(flag==1){
   			...
   			int j=0;
%>
<form action="Modify" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
<div class="outer">
	<div class="basic">
		<div class="pic">
			<img alt="" src=<%=session.getAttribute("Image") %>>
			<input type="file" title="上传头像" class="avatar" name="avatar" value=<%=session.getAttribute("Image") %> id="uld">
			<input type="text" id="temp" name="temp" value="0">
<script type="text/javascript">
var button1 = document.getElementById("uld");
...
// 为按钮元素的属性onclick事件设置一个处理函数
			...
			<img id="upload" alt="" src="./images/icons/camera.png">
		</div>
		<div class="uname">
			<input class="text" type="text" name="unm" value=<%=username %>> 
		</div>
		<div class="identity">身份:教师</div>
		<button id="confirmi" type="submit" onclick="return confirm('确认修改?')">确认</button>
		<a href="detail.jsp" class="edit" target="_self">取消</a>
	</div>
	<div class="details">
		<div id="ShowOption">
	...
	<svg t="1717653762921" class="icon notice" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5111" width="200" height="200"><path d="M766.598 75.624c-112.55-65.312-250.07-87.553-385.693-51.212C110.437 96.883-50.07 374.89 22.4 645.357c72.472 270.468 350.479 430.975 620.946 358.504 270.467-72.472 430.975-350.479 358.503-620.946-0.211-0.789-0.424-1.577-0.639-2.363h-62.505a454.786 454.786 0 0 1 5.189 17.892c63.895 238.46-77.618 483.566-316.077 547.461-238.46 63.895-483.566-77.617-547.461-316.077-63.895-238.46 77.617-483.566 316.077-547.46 119.62-32.053 240.912-12.416 340.164 45.217l-42.383 73.409 217.66-6.997L809.104 2l-42.507 73.624z" fill="#000000" p-id="5112"></path><path d="M442 447h140v-70c0-38.66-31.34-70-70-70s-70 31.34-70 70v70z m44 186.012V716h52v-82.988c25.786-10.321 44-35.54 44-65.012 0-38.66-31.34-70-70-70s-70 31.34-70 70c0 29.472 18.214 54.69 44 65.012zM390 447v-70c0-67.379 54.621-122 122-122 67.379 0 122 54.621 122 122v70h37c18.778 0 34 15.222 34 34v252c0 18.778-15.222 34-34 34H353c-18.778 0-34-15.222-34-34V481c0-18.778 15.222-34 34-34h37z" fill="#000000" p-id="5113"></path></svg>
		...
		<div class="data">
			...
			<div class="content">
				<div><span>地址:</span><input class="location" type="text" name="location" value=<%="'"+location+"'" %>> </div>
				<div><span>电话号码:</span><input class="phone" type="text" name="phonenumber" value=<%=phonenumber %>> </div>
				<div><span>邮箱:</span><input class="email" type="text" name="email" value=<%=email %>> </div>
				<div><span>学历:</span><input class="degree" type="text" name="degree" value=<%=degree %>> </div>
				<% for(i=0;i<6;i++){
				%>
				<div class="course_div"><span>课程<%=i+1%>:</span>
				<input id=<%="course"+i%> class="course" type="text" name=<%="course"+i %> value=<%=courses[i].equals("")?"":courses[i] %>> 
				<select id=<%="sp"+i%> onChange=<%="cg("+i+");"%>>
				<option class=<%="CourseOp"+i%>></option>
				<option class=<%="CourseOp"+i%>>Chinese</option>
				<option class=<%="CourseOp"+i%>>Math</option>
				<option class=<%="CourseOp"+i%>>English</option>
				<option class=<%="CourseOp"+i%>>Physics</option>
				<option class=<%="CourseOp"+i%>>Chemistry</option>
				<option class=<%="CourseOp"+i%>>Computer</option>
				<option class=<%="CourseOp"+i%>>Machinery</option>
				</select>
				</div>
				<%} %>
				<script type="text/javascript">
				function cg(i){
					var demo=document.getElementById("sp"+i).value;
					document.getElementById("course"+i).value=demo;
				}
				</script>
				<div><span>期望薪资(元/小时):</span><input class="salary" type="text" name="salary" value=<%=salary %>></div>
				<% for(j=0;j<12;j+=2){
					String[] ftm=freetime[j].split(":",-1);
					String weekday=ftm[0];
					String hour=ftm[1];
					String minute=ftm[2];
					ftm=freetime[j+1].split(":",-1);
					String weekday1=ftm[0];
					String hour2=ftm[1];
					String minute3=ftm[2];
					int tpqi=j+1;
				%>
				<div class="time_div">
				<span>空闲时间<%=j/2+1%>:</span>
				<div>
				<input id=<%="weekday"+j%> class="time" type="text" name=<%="beginw"+(j/2) %> value=<%=weekday %>> 
				<select class="time_select" id=<%="spw"+j%> onChange=<%="cgw("+j+");"%>>
				<option ></option>
				...
				<span id="snatch">:</span>
				<div>
				<input id=<%="hour"+j%> class="time" type="text" name=<%="beginh"+(j/2) %> value=<%=hour %>> 
				<select class="time_select" id=<%="sph"+j%> onChange=<%="cgh("+j+");"%>>
				<option ></option>
				...
				<span id="snatch">:</span>
				<div>
				<input  id=<%="miniute"+j%> class="time" type="text" name=<%="beginm"+(j/2) %> value=<%=minute %>> 
				<select class="time_select" id=<%="spm"+j%> onChange=<%="cgm("+j+");"%>>
				<option ></option>
				...
				<span id="snatch">~</span>
				<div>
				<input id=<%="weekday"+tpqi%> class="time" type="text" name=<%="endw"+j/2 %> value=<%=weekday1 %>> 
				<select class="time_select" id=<%="spw"+tpqi%> onChange=<%="cgw("+tpqi+");"%>>
				<option ></option>
				<option >Monday</option>
				<option >Tuesday</option>
				<option >Wednesday</option>
				...
				<span id="snatch">:</span>
				<div>
				<input id=<%="hour"+tpqi%> class="time" type="text" name=<%="endh"+j/2 %> value=<%=hour2 %>> 
				<select class="time_select" id=<%="sph"+tpqi%> onChange=<%="cgh("+tpqi+");"%>>
				<option ></option>
				<%for(int hp=0;hp<24;hp++){
					%>
					<option><%=hp %></option>
				...
				<span id="snatch">:</span>
				<div>
				<input id=<%="miniute"+tpqi%> class="time" type="text" name=<%="endm"+j/2 %> value=<%=minute3 %>> 
				<select class="time_select" id=<%="spm"+tpqi%> onChange=<%="cgm("+tpqi+");"%>>
				<option ></option>
				<%for(int mp=0;mp<60;mp++){
					%>
					<option><%=mp %></option>
				...
				<script type="text/javascript">
				function cgw(i){
					var demo=document.getElementById("spw"+i).value;
					document.getElementById("weekday"+i).value=demo;
				}
				function cgh(i){
					var demo=document.getElementById("sph"+i).value;
					document.getElementById("hour"+i).value=demo;
				}
				function cgm(i){
					var demo=document.getElementById("spm"+i).value;
					document.getElementById("miniute"+i).value=demo;
				}
				</script>
			</div>
		</div>
