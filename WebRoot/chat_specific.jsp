		...
		<div class="col header__inner">
	        <div class="col header__navwrap">
	         ...
	         <div class="col header__right">
	         	...
	         	<% 
	         	String toid=request.getParameter("chat_to_id");
	        	//String toid="218975251203043";
	        	String updsql="update message set readed = 0 where fromid= '"+toid+"' and toid='"+session.getAttribute("uid")+"'";
	        	String driverClassT = "com.mysql.jdbc.Driver";
	        	String urlT = "jdbc:mysql://localhost:3306/project?useSSL=false&serverTimezone=UTC";
	        	String urnT = "root";
	        	String pswT = "AnXin202511408!";
	        	Class.forName(driverClassT);//加载驱动 
	        	Connection connT = DriverManager.getConnection(urlT,urnT,pswT);//得到连接
	        	if(connT !=null){
	        		Statement stm = connT.createStatement();
	        		stm.execute(updsql);
	        	}
...
<form action="chat_specific.jsp" name="form1">
	<input id="chat_special" name="chat_to_id" value="">
	<div id="chatbox">
		<div id="chatpage">
			<div id="chatsign">
			...
			<div>
				...
				<div id="chat_to_user">
				<%
				String sp_tmp="select *  from usr where uid='"+toid+"'";
				stmt = conn.createStatement();
        		ResultSet rs_stp =stmt.executeQuery(sp_tmp);
        		rs_stp.next();
				%>
				<span id="who_i_chat_to"><%=rs_stp.getString("uname") %></span>
				</div>
				<div id="chat_details">
					<%
					String details_sql="select * from message where (fromid='"+session.getAttribute("uid")+"' and toid='"+toid+"') or (fromid='"+toid+"' and toid='"+session.getAttribute("uid")+"')";
					Statement stmtC = conn.createStatement();
            		ResultSet rs_urC =stmtC.executeQuery(details_sql);
            		int cunot=0;
            		while(rs_urC.next()){
            			%>
            			<div class="chat_page_item"  >
            			
            			<%if(rs_urC.getString("fromid").equals(session.getAttribute("uid"))){
            				%>
            				<img class="chat_item_image_send" src=<%=session.getAttribute("Image") %>>
            				<a class="chat_message_form_send" href="view.jsp?mid=<%=rs_urC.getString("mid") %>&sor=<%=rs_urC.getString("FromOrTo") %>&fot=1&chat_to_usr=<%=toid %>" id=<%=("item"+cunot) %>>您向对方发送了一条消息,点击查看内容</a>
            				<%
            			}
            			else{
            				%>
            				<a class="chat_message_form_receive" href="view.jsp?mid=<%=rs_urC.getString("mid") %>&sor=<%=rs_urC.getString("FromOrTo") %>&fot=0&chat_to_usr=<%=toid %>" id=<%=("item"+cunot) %>>对方向您发送了一条消息,点击查看内容</a>
            				<img class="chat_item_image_receive" src=<%=rs_stp.getString("addr") %>>
            				<%
            			}
            			%>
